...
export const ShapeContainer = styled.div<{
    ...
    position: relative;
    ${props => (props.$isDragging || props.$returningToOrigin) && `
    ...
    &:hover {
        transform: ${props => !props.$isDragging && !props.$returningToOrigin && props.$draggable ? 'scale(1.05)' : props.$isDragging || props.$returningToOrigin ? `translate(${props.$translateX}px, ${props.$translateY}px)` : 'none'};
        box-shadow: ${props => !props.$isDragging && !props.$returningToOrigin && props.$draggable ? '0 5px 15px rgba(0, 0, 0, 0.1)' : 'none'};
    }
