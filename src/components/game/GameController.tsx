 ...
 * @todo Рефакторинг: разделить на более мелкие компоненты
 */
...
export const GameController: FC<Props> = ({ onShare, onMainMenu }) => {
    ...
    const handleDragEnd = useCallback(
        (clientX?: number, clientY?: number) => {
            ...
            if (!effectiveShape && draggingShape && shapes.length > 0) {
                console.log(
                    'No selected shape, but dragging is active. Trying to find shape by ID:',
            ...
            try {
                if (
                    clientX !== undefined &&
                    clientY !== undefined &&
                    (gameState.selectedShape || selectedShapeRef.current) &&
                    gridRef.current
                ) {
                    const gridRect = gridRef.current.getBoundingClientRect();
                    const buffer = 20;
                    ...
                    if (
                        ...
                        if (check.valid) {
                            ...
                            for (
                                ...
                                r <= Math.min(row + 1, gameState.grid.length - 1);
                                r++
                            ) {
                                for (
                                    let c = Math.max(0, col - 1);
                                    c <= Math.min(col + 1, gameState.grid[0].length - 1);
                                    c++
                                ) {
                                    const shapeToUse =
                                        gameState.selectedShape || selectedShapeRef.current;
                                    ...
                                    const nearbyCheck = canPlaceShape(
                                        gameState.grid,
                                        nonNullShape,
                                        r,
                                        c
